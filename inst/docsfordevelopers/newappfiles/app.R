...
#use this as template for your own App
#replace content below with your content
############################################################
...
refresh <- function(input, output)
  ...
  output$plot  <- renderPlot({
    ...
    DSAIRM::generate_plots(res) #create plots with a non-reactive function
  ...
  output$text <- renderText({
    input$submitBtn
    res=isolate(result()) #list of all results that are to be turned into plots
    DSAIRM::generate_text(res) #create text for display with a non-reactive function
  })
...
ui <- fluidPage(
  includeCSS("dsairm.css"), #to get the look correct
  ...
  do.call(tabsetPanel,DSAIRM::generate_documentation())
