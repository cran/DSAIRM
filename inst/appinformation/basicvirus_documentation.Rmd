...
_0 = n - d~U~ U - bUV_, _0 = bUV - d~I~_, _0 = pI - d~V~ V - gb UV_. One can solve those equations for each of the compartments to get a mathematical expression of what _U_, _I_ and _V_ are at steady state. Try to do this. You should find that _U~s~ = d~I~ d~V~/(b(p - d~I~ g))_ and similar equations for _I~s~_ and _V~s~_ (the subscript _s_ denotes that these are the steady-state values of the variables).
* Check that your equations agree with the simulations. Plug the values for the parameters into each of the equations and see if the steady state values _U~s~_, _I~s~_ and _V~s~_ you computed with the equations is the same as you get as steady state value from the simulation. If that's not the case, it means your equations aren't right yet.
